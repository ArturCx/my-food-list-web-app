    ...
    <div className="flex flex-col gap-4 p-4 pb-6">
      ...
      <div className="animate-in fade-in zoom-in-95 relative duration-300">
        ...
        {onClose && (
          <button
            ...
            onClick={onClose}
            aria-label="Fechar"
            className="absolute top-3 right-3 flex size-10 items-center justify-center rounded-full bg-white/85 dark:bg-white/20 shadow-md backdrop-blur transition-transform hover:scale-105 active:scale-95"
          >
            <X className="size-4" />
          </button>
        )}
      ...
      <div className="animate-in fade-in slide-in-from-bottom-2 px-1 duration-300 [animation-delay:60ms]">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {r.categories.map((c) => {
            ...
            return (
              <span key={c} className="flex items-center gap-1.5 rounded-full bg-white/70 dark:bg-white/12 py-1 pr-2.5 pl-1.5 text-[11px] font-bold">
      ...
      <div className="animate-in fade-in slide-in-from-bottom-2 grid grid-cols-2 gap-2.5 duration-300 [animation-delay:180ms]">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="glass-soft group rounded-2xl p-3 transition-[background-color,transform] hover:bg-white/85 dark:bg-white/20 active:scale-[0.98]"
        >
      ...
      {r.instagramEmbed && (
        <div className="animate-in fade-in overflow-hidden rounded-[20px] bg-white/50 dark:bg-white/12 duration-300 [animation-delay:300ms]">
    ...
    <a
      ...
      className={cn(
        ...
        primary
          ...
          : "glass-soft hover:bg-white/85 dark:bg-white/20 hover:shadow-[0_6px_16px_rgba(15,23,42,0.1)]",
      )}
    >
      {children}
    </a>
